...
// Weapon and Combat Uniforms
uniform float u_fire; // 0 or 1
uniform vec3 u_missile;
uniform vec3 u_drone1;
...
float map(vec3 p) {
    ...
    float dFlap = length(max(abs(pFlap) - vec3(0.4, 0.1, 0.3), 0.0));

    // Sharp faceted blend for stealth geometry
    float d = min(dBody, min(dEng, dFlap));

    // Missile
    if (u_missile.z < 0.0) {
        float dMis = length(p - u_missile) - 0.15;
        d = min(d, dMis);
    }
    
    // Drones
    if (u_drone1.z < 0.0) {
        float dDrone1 = length(p - u_drone1) - 0.4;
        d = min(d, dDrone1);
    }
    if (u_drone2.z < 0.0) {
        float dDrone2 = length(p - u_drone2) - 0.4;
        d = min(d, dDrone2);
    }
...
void main() {
    ...
    if(t < MAX_DIST) {
        ...
        float dMis = u_missile.z < 0.0 ? length(p - u_missile) - 0.15 : 1e6;
        ...
        if (dMis < 0.05) {
        ...
        } else if (dDrone1 < 0.05 || dDrone2 < 0.05) {
            // Drone shading
            float diff = max(dot(n, l), 0.0);
            col = vec3(0.3, 0.1, 0.1) * diff; // dark red
            col += vec3(1.0, 0.0, 0.0) * pow(1.0 - max(dot(-rd, n), 0.0), 2.0); // red rim
        } else {
            // Basic lighting for ship
            float diff = max(dot(n, l), 0.0);
            float amb = 0.1;
            col = vec3(0.12, 0.13, 0.15) * (diff + amb); // Matte obsidian gray base
            
            // Rim lighting (neon-cyan)
            float rim = pow(1.0 - max(dot(-rd, n), 0.0), 4.0);
            col += vec3(0.0, 0.8, 1.0) * rim * 0.8;

            // Calculate local coordinates for emissive mapping
            vec3 localP = (inverse(u_shipRot) * vec4(p, 1.0)).xyz;

            // Emerald green circuitry trims (procedural pattern on ship body)
            float circuitry = smoothstep(0.95, 1.0, sin(localP.x * 40.0) * cos(localP.z * 30.0 + u_time * 2.0));
            col += vec3(0.0, 1.0, 0.4) * circuitry * 3.5;
            ...
            float pulseGlow = pulseShape * 3.5 * (1.0 - pulseProgress); 

            float wave1 = sin(localP.z * 8.0 - u_time * 20.0) * 0.5 + 0.5;
            float wave2 = sin(localP.z * 5.0) * 0.5 + 0.5;
            float sustainedIntensity = 0.4 + pow(wave1, 2.0) * 1.5 + pow(wave2, 4.0) * 1.0;
            ...
            col += vec3(0.1, 0.4, 1.0) * engineMask * totalGlow * 1.5;
    ...
    // Add Flamethrower overlay independently of surface hit
    if (u_fire > 0.5) {
        // Simple volumetric cone for plasma fire
        // Ship is roughly at (0,0,-1 to 1)
        // Fire is projected forward (-Z)
        vec3 pFire = ro + rd * min(t, 10.0); 
        // distance to line from (0,0,0) to (0,0,-10)
        float dLine = length(pFire.xy);
        float zMask = smoothstep(0.0, -6.0, pFire.z);
        float fireWidth = 0.5 + abs(pFire.z) * 0.4;
        if (dLine < fireWidth && pFire.z < 0.0) {
            float fireIntensity = smoothstep(fireWidth, 0.0, dLine) * zMask * (1.0 + sin(u_time * 50.0) * 0.2);
            vec3 fireCol = vec3(1.0, 0.3, 0.0) * fireIntensity;
            outColor.rgb += fireCol;
            outColor.a = max(outColor.a, fireIntensity);
        }
    }

    // Add Explosions
    if (u_explosion1 > 0.0) {
        float dE = length(ro + rd * length(u_drone1 - ro) - u_drone1);
        float eIntensity = smoothstep(1.5, 0.0, dE) * u_explosion1;
        outColor.rgb += vec3(1.0, 0.5, 0.1) * eIntensity * 2.0;
        outColor.a = max(outColor.a, eIntensity);
    }
    if (u_explosion2 > 0.0) {
        float dE = length(ro + rd * length(u_drone2 - ro) - u_drone2);
        float eIntensity = smoothstep(1.5, 0.0, dE) * u_explosion2;
        outColor.rgb += vec3(1.0, 0.5, 0.1) * eIntensity * 2.0;
        outColor.a = max(outColor.a, eIntensity);
    }
}
...
export const ShipOverlay: React.FC = () => {
    ...
    const combatState = useRef({
        fire: 0,
        missile: [0, 0, 10], // Initially hidden behind camera (z=10)
        drone1: [-2, 0, -15],
        drone2: [2, 0, -20],
        explosion1: 0,
        explosion2: 0,
    ...
    useEffect(() => {
        ...
        const locs = {
            ...
            // Weapons
            uFire: gl.getUniformLocation(p, 'u_fire'),
            uMissile: gl.getUniformLocation(p, 'u_missile'),
        ...
        const render = (t: number) => {
            ...
            if (pressedKeysRef.current.has(' ')) {
                cs.fire = Math.min(1.0, cs.fire + dt * 5.0);
            } else {
                cs.fire = Math.max(0.0, cs.fire - dt * 5.0);
            }

            // Missile Launch
            if (pressedKeysRef.current.has('m') && !cs.missileActive) {
                cs.missileActive = true;
                cs.missile = [0, -0.2, 0]; // Start at ship bay
                cs.missileVelocity = 2.0; // Initial velocity
            }

            // Drone Movement (Approaching ship)
            cs.drone1[2] += 10.0 * dt;
            cs.drone2[2] += 12.0 * dt;
            ...
            if (cs.missileActive) {
                ...
                // Collision Detection Drone 1
                if (cs.explosion1 === 0 && Math.abs(cs.missile[2] - cs.drone1[2]) < 1.0 && Math.hypot(cs.missile[0] - cs.drone1[0], cs.missile[1] - cs.drone1[1]) < 2.0) {
                    cs.explosion1 = 1.0;
                    cs.missileActive = false;
                    cs.missile = [0, 0, 10]; // Hide
                }
                // Collision Detection Drone 2
                if (cs.explosion2 === 0 && cs.missileActive && Math.abs(cs.missile[2] - cs.drone2[2]) < 1.0 && Math.hypot(cs.missile[0] - cs.drone2[0], cs.missile[1] - cs.drone2[1]) < 2.0) {
                    cs.explosion2 = 1.0;
                    cs.missileActive = false;
                    cs.missile = [0, 0, 10]; // Hide
                }
            ...
            // Flamethrower Collision (Range ~ -10Z)
            if (cs.fire > 0.5) {
                if (cs.drone1[2] > -10.0 && cs.drone1[2] < 0.0 && Math.abs(cs.drone1[0]) < 2.0) {
                    cs.explosion1 = 1.0;
                }
                if (cs.drone2[2] > -10.0 && cs.drone2[2] < 0.0 && Math.abs(cs.drone2[0]) < 2.0) {
                    cs.explosion2 = 1.0;
                }
            }

            // Explosions
            if (cs.explosion1 > 0.0) {
                cs.explosion1 += dt * 2.0;
                if (cs.explosion1 > 3.0) { cs.explosion1 = 0; cs.drone1 = [(Math.random()-0.5)*15, (Math.random()-0.5)*5, -30 - Math.random()*20]; }
            }
            if (cs.explosion2 > 0.0) {
                cs.explosion2 += dt * 2.0;
                if (cs.explosion2 > 3.0) { cs.explosion2 = 0; cs.drone2 = [(Math.random()-0.5)*15, (Math.random()-0.5)*5, -30 - Math.random()*20]; }
            }

            // Respawns on miss
            if (cs.drone1[2] > 5) cs.drone1 = [(Math.random()-0.5)*15, (Math.random()-0.5)*5, -30 - Math.random()*20];
            if (cs.drone2[2] > 5) cs.drone2 = [(Math.random()-0.5)*15, (Math.random()-0.5)*5, -30 - Math.random()*20];
            ...
            // Combat uniforms
            gl.uniform1f(locs.uFire, cs.fire);
            gl.uniform3fv(locs.uMissile, cs.missile);
            gl.uniform3fv(locs.uDrone1, cs.drone1);
